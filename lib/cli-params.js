...
function getCliParams(argv) {
  ...
  const port = args.port || process.env.PORT || DEFAULT_PORT
...
The default port if not specified will be ${DEFAULT_PORT}.
...
use 127.0.0.1, meaning the server can only be accessed by clients on the same
machine.  To allow any machine to access the server, use the --anyAddress flag,
which will bind instead to 0.0.0.0.  Or you can bind to a specific address with
the --address flag.
